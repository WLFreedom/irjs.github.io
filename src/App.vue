    ...
    <v-locale-provider>
      ...
      <v-footer class="position-fixed d-flex justify-space-between">
        ...
        <div class="d-flex align-center">
          <v-select @update:modelValue="languageChanged" menu-icon="mdi-chevron-down" prepend-inner-icon="mdi-translate-variant" color="primary" single-line density="compact" v-model="language" :items="store.getters.languages" item-title="name" item-value="code"></v-select>
        </div>
        <div class="ml-4">
          <v-btn href="https://github.com/irjs/irjs.github.io/blob/master/README.md" size="small" color="primary">{{$t('guide_contribute_in_data')}}</v-btn>
        </div>
      </v-footer>
    </v-locale-provider>
  </v-app>
